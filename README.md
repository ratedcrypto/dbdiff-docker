# DBDiff-docker: Docker setup for DBDiff
...
server1:
    ...
    password: root
    port: 3306
    host: mysql56 # docker-compose service name
server2:
    user: root
    password: root
    port: 3306
...
Under initdb directory add db1.sql and db2.sql to compare. 

```bash
# Start up containers
make start

# Wait for php container install composer dependencies and mysql to start
# To run the db diff
# This will create migration.sql under public directory
make diff
...
To resolve MySQL connectivity issues comment out try-catch `public/src/DB/DBManager.php/testResource` so it stops throwing this exception and starts showing proper errors.
